...
class WeFitterHealthConnectModule(var reactContext: ReactApplicationContext) : NativeRTNWeFitterHealthConnectSpec(reactContext) {
    ...
    override fun configure(config: ReadableMap) {
        ...
        val statusListener = object : WeFitterHealthConnect.StatusListener {
            override fun onConfigured(configured: Boolean) {
                emitOnConfiguredWeFitterHealthConnect(configured)
            }

            override fun onConnected(connected: Boolean) {
                emitOnConnectedWeFitterHealthConnect(connected)
            }

            override fun onError(error: WeFitterHealthConnectError) {
                emitOnErrorWeFitterHealthConnect(error.message)
            }
        }
    ...
    fun connect2() {
        ...
        if (weFitter.isConnected())
            ...
            emitOnErrorWeFitterHealthConnect("ERROR CONNECTING")
        }
    }
    ...
    override fun isConnected(): Boolean {
        return weFitter.isConnected()
    }

    override fun isSupported() : Boolean {
        return weFitter.isSupported()
    }
